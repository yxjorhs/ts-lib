 ...
 *
 * 使用：见RedisCacheManager.test.ts，就三函数: get, mget, refresh
 *
 * 目的:
 * 1. 通过{expireIn}配置所有缓存的过期时间，缓存在用到时会刷新过期时间(所以说没人用数据就丢了)
 * 2. 配置{cache}是个对象，保存缓存注册配置，name为缓存名
 * 3. 使用{cache[name].r}配置缓存来源，缓存不存在时通过r()获取数据并缓存
 * 4. 缓存来源r()允许传一个参数(多个自个拼成个字符串);没参数的使用公用的哈希表存储数据、name作为member；有参数独自开个哈希表，参数作为member
 * 5. 提供了mget函数，以管道的形式、可以一次性获取保存在多个哈希表中的数据
 * 6. 通过事件机制刷新缓存，使用{re}注册缓存的刷新事件，收到事件便会刷新对应的缓存(例如：当哪个表发生修改的时候，把表名传进来，就不用调del函数删缓存了)
 */
